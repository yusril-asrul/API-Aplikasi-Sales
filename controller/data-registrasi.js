const { loadDataRegistrasi, loadDataRegistrasiAll, simpanUpdateRegistrasi, cekDataRegistrasi, loadDataAktifitas, hapusAktifitas, loadDataKebutuhan, hapusKebutuhan } = require('../models');
...
const load_all_user = async function (req, res) {
    try {
        let id = req.params.id ? req.params.id : '';
        const { cari } = req.body

        if (!cari || cari.trim() === '') {
            return responseError(res, 'Kata kunci pencarian tidak boleh kosong.', 400);
        }

        if (cari.length < 3) {
            return responseError(res, 'Kata kunci pencarian minimal 3 karakter.', 400);
        }

        let data = await loadDataRegistrasiAll(cari);
        if (data) {
            return responseSuccess(res, 'Loaded.', data)
        } else {
            return responseSuccess(res, 'data not found.', {}, false)
...
module.exports = { load, update_status, update_demo, load_aktfitas, simpan_aktifitas, hapus_aktiftas, update_status_aktifitas, load_kebutuhan, simpan_kebutuhan, hapus_kebutuhan, load_all_user }